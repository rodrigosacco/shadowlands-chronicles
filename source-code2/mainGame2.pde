...
ArrayList<Bullet2> bulletList;
ArrayList<Enemy2> enemyList;
...
//SoundFile skull;
  ...
  GameScreen2() {
    //initialize my vars
  p1 = new Player2(width/2, height/2, 40, 50);

  
  bulletList = new ArrayList<Bullet2>();
  enemyList = new ArrayList<Enemy2>();
  ...
  if(tempoRestante > 0 && frameCount % 60 == 0 && tempoRestante % intervaloSpawn == 0){
    for (int i = 0; i < quantidadeInimigos; i++){
      ...
      enemyList.add(new Enemy2(startingX, startingY, startingW, startingH));
    }
  }
  ...
  for (Bullet2 aBullet : bulletList){
    aBullet.render();
    aBullet.move();
    aBullet.checkRemove();
  ...
  for (int i = bulletList.size() - 1; i >= 0; i = i - 1) {
    Bullet2 aBullet = bulletList.get(i);
   ...
   for (int i = enemyList.size() - 1; i >= 0; i = i - 1) {
    Enemy2 anEnemy = enemyList.get(i);
...
void keyPressed() {
  ...
  if (p1.hasMoved) {
    ...
    bulletList.add(new Bullet2(bulletX, bulletY, bulletDirX, bulletDirY));
  }  
}
